...
class AplicarEncuestaEmailController extends Controller
{
    public function mostrar_encuesta_aplicar($id_encuesta=0)
    {
        $id = decrypt($id_encuesta);
        $encuesta = App\DEncuestas::where('id', $id)->get();
        $estilos_encuesta = App\DEstiloEncuesta::where('encuesta_id', $id)->get();
        return view('encuesta.email.index', compact('encuesta','estilos_encuesta'));
    }

    public function get_datos_preguntas($id_encuesta= 0)
    {
    	 try {
    ...
    public function guardar_datos_encuesta(Request $request)
     {
        try {
            $respuestas = $request->all();

            $user_id = DB::table('d_encuestas')->where('id',  $respuestas[0]['id_encuesta'])->value('usuario_id');
    ...
    public function guardar_datos_archivo(Request $request)
    {
    ...
    public function mensaje_encuesta_aplicado()
    {
    	# code...
    }
}
