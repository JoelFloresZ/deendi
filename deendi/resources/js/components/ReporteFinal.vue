<template>
    <div class="p-4 w-100" >
        <button type="button" name="" id="" class="btn btn-primary" @click="pdf">
            PDF
        </button>
        ...
        <div v-for="pregunta in resultados">
            ...
            <div v-if="pregunta.tipo_pregunta === 'pre_archivo'">
                <label class="font-weight-bold">{{pregunta.pregunta}}</label>
                <div v-html="pregunta.contenido"></div> 

                <!-- Modal -->
                <div class="modal fade" :id="pregunta.id_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalScrollableTitle">Galeria de imagenes</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          <span aria-hidden="true">&times;</span>
                        </button>
                      </div>
                      <div class="modal-body">
                        <div v-for="imagen in pregunta.resultados">
                            <figure>
...
<script>
    ...
    export default {
        ...
        pdf() {
            ...
            , font, lines
            , altura = 0.5 // inches on a 8.5 x 11 inch sheet.
            , loremipsum = 'Lorem ipsum dolor sit amet, ...'

            font = fonts[0]
            var sizeTitulo = sizes[1]
            var sizeDescripcion = sizes[0]

            /* Titulo */
            lines = pdf.setFont(font[0], font[1]).setFontSize(sizeTitulo).splitTextToSize(encuesta[0].titulo, 7.5)
            pdf.text(0.5, altura + sizeTitulo / 72, lines)
            altura += (lines.length + 0.5) * sizeTitulo / 72

             /* Descripción */
            lines = pdf.setFont(font[0], font[1]).setFontSize(sizeDescripcion).splitTextToSize(encuesta[0].descripcion, 7.5)
            pdf.text(0.5, altura + sizeDescripcion / 72, lines)
            altura += (lines.length + 1) * sizeDescripcion / 72

           /* Preguntas */
            var preguntas =  encuesta[0].resultados[0];
            for(var i = 0; i < preguntas.length; i++ ){
                var pregunta = preguntas[i].pregunta;
                var tipo_pregunta = preguntas[i].tipo_pregunta;

                lines = pdf.setFont(font[0], font[1]).setFontSize(sizeDescripcion).splitTextToSize(pregunta, 7.5)
                pdf.text(0.5, altura + sizeDescripcion / 72, lines)
                altura += (lines.length + 0.5) * sizeDescripcion / 72
                
                /* Respuestas */
                if(tipo_pregunta === "pre_abierta"){
                    var respuestas = preguntas[i].resultado;
                    for(var x = 0; x < respuestas.length; x++ ){
                        var respuesta = respuestas[x].respuesta;
                        lines = pdf.setFont(font[0], font[1]).setFontSize(sizeDescripcion).splitTextToSize(respuesta, 7.5)
                        pdf.text(0.5, altura + sizeDescripcion / 72, lines)
                        altura += (lines.length + 0.5) * sizeDescripcion / 72

                       /*  lines = pdf.setFont(font[0], font[1]).setFontSize(sizeDescripcion).splitTextToSize(`${Math.round(altura)} hola`, 7.5)
                        pdf.text(0.5, altura + sizeDescripcion / 72, lines)
                        altura += (lines.length + 0.5) * sizeDescripcion / 72 */

                        var nextPage = 10; 
                        if(Math.round(altura) > nextPage){
                            pdf.addPage();
                            nextPage +=10;
                            altura = 0.5
                        }
