<template>
    <div>
        <form method="POST" v-on:submit.prevent="enviarDatos" enctype="multipart/form-data">
            <!-- <h2 class="font-weight-bold color_titulo text-shadow">{{encuesta.titulo}}</h2>
            ...
            <div class="d-flex justify-content-start w-75 w-md-100">
                <button class="btn btn-outline-primary" style="border-radius: 20px;">ENVIAR</button>
            </div>
        </form>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...Vuex.mapState(['url_encuesta_recibido']) //Se mapea los States deL Store
        },
        ...
        methods: {
            ...
            enviarDatos() {
                ...
                axios.post(`${this.url_encuesta_recibido}/guardar/datos`, respuestas).then((result) => {
                    //console.log(result.data);
                    if (result.data === "success") {
                       $('#model2Id').modal('hide')
                       document.location.replace(`${this.url_encuesta_recibido}/encuesta/finalizado`);
            ...
            enviarDatosArchivo(datos) {
                ...
                $.ajax({
                    url:`${this.url_encuesta_recibido}/guardar/archivos`,
